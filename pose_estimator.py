...
class PoseEstimator:
    ...
    def get_pose(self) -> Optional[np.ndarray]:
        ...
        # If more than 6 blobs, keep the top 6 by area
        if len(blob_stats) > 6:
            blob_stats = sorted(blob_stats, key=lambda x: x["area"], reverse=True)[:6]
        ...
        if len(blobs_position) == 6:
            # Sort blobs by y-coordinate (ascending)
            sort_y = sorted(blobs_position, key=lambda x: x[1])

            # Identify top, middle, and bottom blobs
            top_right = sort_y[0]
            sort_middle = sorted(sort_y[1:4], key=lambda x: x[0])
            middle_left, middle_center, middle_right = sort_middle
            sort_bottom = sorted(sort_y[4:], key=lambda x: x[0])
            bottom_left, bottom_right = sort_bottom
            ...
            # Define the corresponding object points in the real world
