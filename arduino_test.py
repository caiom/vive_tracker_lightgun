...
class ArduinoAbsMouse:
    def __init__(self):
        self.arduino = serial.Serial(port='COM8', baudrate=115200)
    def move_mouse(self, x: float, y: float, mouse_buttom:int=0, aspect_43=False):

        if aspect_43:
            x = np.clip(x, 0.125, 0.875)
            x = (x-0.125) / 0.75
            
        x = np.clip(x, 0.0, 1.0)
        y = np.clip(y, 0.0, 1.0)
        self.arduino.write(struct.pack('<BffBB', 37, x, y, mouse_buttom, mouse_buttom))
    def close(self):
        self.arduino.close()
...
#                 self.mouse.sendMouseReport(moviment_x, moviment_y, mouse_buttom)
#                 last_mouse_buttom = mouse_buttom
#                 with self.thread_lock:
#                     self.pos_x += moviment_x * (1/sensi)
#                     self.pos_y += moviment_y * (1/sensi)
